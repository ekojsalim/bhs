...
eval (List [Atom "jika", pred, truExpr, flsExpr]) = do
  ifRes        <- eval pred
  case ifRes of
    (Bool True)  -> eval truExpr
    (Bool False) -> eval flsExpr
    _            -> throw $ BadSpecialForm "if's first arg must eval into a boolean"
eval args@(List ( (:) (Atom "jika") _))  = throw $ BadSpecialForm "(jika <bool> <s-expr> <s-expr>)"

eval (List [Atom "mulai", rest]) = evalBody rest
eval (List ((:) (Atom "mulai") rest )) = evalBody $ List rest

eval (List [Atom "misal", varExpr, defExpr]) = do --top-level define
  EnvCtx{..} <- ask
  varAtom <- ensureAtom varExpr
  evalVal <- eval defExpr
  bindArgsEval [varExpr] [defExpr] varExpr

eval (List [Atom "anggap", List pairs, expr]) = do
  EnvCtx{..} <- ask
  atoms <- mapM ensureAtom $ getEven pairs
  vals  <- mapM eval       $ getOdd  pairs
  bindArgsEval atoms vals expr
eval (List (Atom "anggap" : _)) = throw $ BadSpecialForm "misal function expects list of parameters and S-Expression body\n(misal <pairs> <s-expr>)"
...
evalBody x@(List [List ((:) (Atom "misal") [Atom var, defExpr]), rest]) = do
  evalVal <- eval defExpr
  ctx <- ask
